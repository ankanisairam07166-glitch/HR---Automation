...
async def extract_invite_link_from_assessment(assessment_name):
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            try:
                assessment_link = await page.wait_for_selector(f"text={assessment_name}", timeout=10000)
                if assessment_link:
            ...
            # --- Dismiss any modal/popups (critical for production!) ---
            try:
                modal_selectors = [
                    '.modal', '.el-dialog', '.popup', '.ant-modal', '.ant-modal-root',
                    '.el-overlay', '.ant-modal-mask', '.modal-backdrop'
                ]
                found_modals = []
                for selector in modal_selectors:
                    modals = await page.query_selector_all(selector)
                    found_modals.extend(modals)
                for modal in found_modals:
                    close_btn = await modal.query_selector(
                        "button:has-text('Close'), .close, .el-dialog__close, .ant-modal-close")
                    if close_btn:
                        ...
                        try:
                            ...
                            pass
                if found_modals:
                    logging.info(f"Closed {len(found_modals)} modals before invite extraction.")
            except Exception as e:
                logging.info(f"No modal or failed to close modal: {e}")
            # --- end modal dismiss logic ---
            ...
            try:
                ...
                for selector in copy_selectors:
                    try:
                        copy_element = await page.wait_for_selector(selector, timeout=5000)
                        if copy_element:
            ...
            if not candidate_link:
                try:
                    ...
                    try:
                        ...
                        pass
                    for i, element in enumerate(clickable_elements[:20]):
                        try:
...
def get_invite_link(assessment_name):
    """Synchronous wrapper for automation pipeline usage."""
    import asyncio
